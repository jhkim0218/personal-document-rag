...
def create_server(source_directory: str | Path, database_path: str | Path, port: int = 8000, chunking: Chunking | None = None, mode: str = "auto", local_embedding_model: str | None = None, local_reranker_model: str | None = None, ocr: bool = False, ocr_executable: str = 'tesseract', ocr_language: str = 'eng', hwp_executable: str | None = None, pricing_path: str | Path | None = None) -> ThreadingHTTPServer:
    service = RAGService(source_directory, database_path, chunking=chunking, mode=mode, local_embedding_model=local_embedding_model, local_reranker_model=local_reranker_model, ocr=ocr, ocr_executable=ocr_executable, ocr_language=ocr_language, hwp_executable=hwp_executable, pricing_path=pricing_path)
...
def main() -> None:
    ...
    parser.add_argument("--pricing", help="Dated local JSON file with model token rates for cost records")
    ...
    server = create_server(args.data, args.db, args.port, Chunking(args.chunk_strategy, args.chunk_size, args.chunk_overlap), mode=args.mode,
                           ...
                           ocr=args.ocr, ocr_executable=args.ocr_executable, ocr_language=args.ocr_language, hwp_executable=args.hwp_executable, pricing_path=args.pricing)
